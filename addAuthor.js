...
const addAuthor = async function (req, headers, branch, owner, repo) {
  ...
  const avatarFilePath = `avatars/${_.kebabCase(
    author_name
  )}-avatar.${extension}`;
  ...
  const getAuthors = await fetch(
    `https://api.github.com/repos/${owner}/${repo}/contents/content/author.yaml`,
    {
      method: "get",
      headers,
    }
  ...
  const addAvatarFile = await fetch(
    `https://api.github.com/repos/${owner}/${repo}/contents/content/${avatarFilePath}`,
    {
      method: "put",
      headers,
